...
export function MCPServerDetail() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // For OAuth servers, poll for the consent URL (generated by background task)
      if (result.auth_type === "oauth" && !result.connected) {
        setOauthPending(true);
        // Poll for up to 30 seconds for the consent URL to appear
        for (let i = 0; i < 30; i++) {
          ...
          try {
            ...
            if (oauthInfo.pending_callback === false && !oauthInfo.consent_url) {
              // Check if the server actually connected (OAuth may have used cached tokens)
        ...
        setOauthPending(false);
        setError("OAuth consent URL was not generated. Check server logs.");
      }
    } catch (e) {
